...
pks=findpeaks(Hs,'MinPeakProminence',range(Hs)*.5);
%pks=findpeaks(Hs);
...
P=1-1./N;
x=gevinv(P,shape,scale,location);

% calculate return values
T=[5 50 500];
RV=gevinv(1-1./T,shape,scale,location);
...
for j=1:trials
    ...
    scale=parmhat(2);
    location=parmhat(3);
    x_boot(j,:)=gevinv(P,shape,scale,location);
    RV_boot(j,:)=gevinv(1-1./T,shape,scale,location);
end
x_boot=sort(x_boot);
...
title(['Exercise ' num2str(exercise) ', Site ' num2str(site) ' - PoT Generalized Extreme Value - Threshold = ' num2str(thprc) '%'])
